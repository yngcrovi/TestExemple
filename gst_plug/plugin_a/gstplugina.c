...
/* 1. Объявляем структуры ПЕРЕД использованием макросов */
typedef struct _GstPluginA GstPluginA;

/* 2. Объявляем приватную структуру ДО G_DEFINE_TYPE */
typedef struct {
    guint64 frame_counter;
} GstPluginAPrivate;

/* 3. Теперь объявляем тип */
G_DECLARE_FINAL_TYPE(GstPluginA, gst_plugin_a, GST, PLUGIN_A, GstElement)

/* 4. Определяем структуру экземпляра */
struct _GstPluginA {
    GstElement parent;
...
/* 6. Регистрируем тип с приватными данными */
...
//==============================================================
// Обработка данных (цепная функция)
//==============================================================
static GstFlowReturn gst_plugin_a_chain(
  ...
  GstPluginAPrivate *priv = gst_plugin_a_get_instance_private(plugin);
    ...
    // 1. Сначала добавляем метаданные к буферу   
    MyCustomMeta *meta = (MyCustomMeta *)gst_buffer_add_meta(
        buffer,
        MY_CUSTOM_META_INFO,
        NULL  // Параметры инициализации (не используем)
    );

    // 2. Затем заполняем поля метаданных
    if (meta) {
